package A1_MyJava.基础知识.IO流.字符缓冲流;
...
public class TestBuffered_ReaderWriter {
    ...
    public static void goBufferedWriter() throws IOException {
        //创建字符缓冲输入流 来 写入数据
        BufferedWriter bw = new BufferedWriter(new FileWriter("D:\\源代码\\src\\A1_MyJava\\基础知识\\IO流\\字符缓冲流\\测试文件.txt"));
        //写数据
        bw.write("问君能有几多愁,\r\n");
        bw.write("恰似一江春水向东流。\r\n");
        //释放资源
    ...
    public static void goBufferedReader(boolean isCharArray) throws IOException {
        //创建缓冲字符输入流 来 读取数据
        BufferedReader br = new BufferedReader(new FileReader("D:\\源代码\\src\\A1_MyJava\\基础知识\\IO流\\字符缓冲流\\测试文件.txt"));
        //读取数据 两种方法
        if (!isCharArray){
            //一次读取一个字符
            int by;
